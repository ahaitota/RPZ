...
def bayes_posterior_params_normal(x, prior_mu0, prior_nu, prior_alpha, prior_beta):
    ...
    mu0 = ((prior_nu * prior_mu0) + np.sum(x)) / (x.size + prior_nu)
    nu = prior_nu + x.size
    alpha = prior_alpha + x.size / 2
    beta = (prior_beta + np.sum(x**2) / 2 
    + (prior_nu * prior_mu0 ** 2) / 2 
    - ((prior_nu * prior_mu0) 
    + np.sum(x))** 2 / (2 * (prior_nu + x.size)))
...
def bayes_estim_pdf_normal(x_test, x,
    ...
    alpha_hat = alpha_v + 1 / 2
    beta_hat = (x_test**2 / 2 + beta_v 
    + (nu_v * mu0_v**2) / 2
    - (nu_v * mu0_v + x_test)**2  / (2 * (nu_v + 1)))
    nu_hat = nu_v + 1

    pdf = ((1 / np.sqrt(2 * np.pi)) 
    * (np.sqrt(nu_v) * beta_v**alpha_v) / (np.sqrt(nu_hat) * beta_hat**alpha_hat)
    * (spec.gamma(alpha_hat)) / (spec.gamma(alpha_v)))
...
def mle_Bayes_classif(x_test, x_train_A, x_train_C):
    ...
    DA['Prior'] = x_train_A.size / (x_train_A.size + x_train_C.size)
    DC['Prior'] = x_train_C.size / (x_train_A.size + x_train_C.size)
...
def bayes_Bayes_classif(x_test, x_train_A, x_train_C,
    ...
    A_prior = x_train_A.size / (x_train_A.size + x_train_C.size)
    C_prior = x_train_C.size / (x_train_A.size + x_train_C.size)

    labels = np.where(pdf_A * A_prior >= pdf_C * C_prior, 0, 1)
    return labels
