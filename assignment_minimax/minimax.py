...
def minimax_strategy_discrete(distribution1, distribution2):
    ...
    r_x = distribution1/distribution2

    size = distribution1.shape[0] * distribution1.shape[1]

    sorted_indices = np.argsort(r_x.flatten())
    rank_array = np.arange(size)
    rank_array[sorted_indices] = np.arange(1, size + 1)
    rank_array = rank_array.reshape(r_x.shape)

    opt_i = 0
    eps1 = np.zeros(size + 1, dtype=float)
    eps2 = np.zeros(size + 1, dtype=float)

    for i in range(0, size + 1):
        # in the begining all of them are 1 class, at the end all are 2
        eps1[i] = np.sum(distribution1[rank_array <= i])
        eps2[i] = np.sum(distribution2[rank_array > i])


    max_array = np.maximum(eps1, eps2)
    opt_i = np.argmin(max_array)

    q = np.where(rank_array <= opt_i, 1, 0)
    q = np.where(r_x == np.nan, 0, q)
...
def classify_discrete(imgs, q):
    ...
    x = (compute_measurement_lr_discrete(imgs) + 10).astype(int)
    y = (compute_measurement_ul_discrete(imgs) + 10).astype(int)
...
def worst_risk_cont(distribution_A, distribution_B, true_A_prior):
    ...
    A['Mean'] = distribution_A['Mean']
    B['Mean'] = distribution_B['Mean']
    A['Sigma'] = distribution_A['Sigma']
    B['Sigma'] = distribution_B['Sigma']
    A['Prior'] = true_A_prior
    B['Prior'] = 1 - true_A_prior

    q = find_strategy_2normal(A, B)

    A['Prior'] = 1
    B['Prior'] = 0

    risk_1 = bayes_risk_2normal(A, B, q)

    A['Prior'] = 0
    B['Prior'] = 1

    risk_2 = bayes_risk_2normal(A, B, q)

    worst_risk = np.max((risk_1, risk_2))
    return worst_risk
...
def minimax_strategy_cont(distribution_A, distribution_B):
    ...
    func = lambda p: worst_risk_cont(distribution_A, distribution_B, p)

    prior=  opt.fminbound(func, 0, 1)

    distribution_A['Prior'] = prior
    distribution_B['Prior'] = 1 - prior

    q = find_strategy_2normal(distribution_A, distribution_B)
    worst_risk = func(prior)
...
def risk_fix_q_cont(distribution_A, distribution_B, distribution_A_priors, q):
    ...
    distribution_B_priors = 1 - distribution_A_priors

    risks = np.zeros_like(distribution_A_priors)
    A = {}
    B = {}

    for i in range(0, distribution_A_priors.shape[0]):
        A['Mean'] = distribution_A['Mean']
        B['Mean'] = distribution_B['Mean']
        A['Sigma'] = distribution_A['Sigma']
        B['Sigma'] = distribution_B['Sigma']
        A['Prior'] = distribution_A_priors[i]
        B['Prior'] = distribution_B_priors[i]
        risks[i] = bayes_risk_2normal(A, B, q)
...
def classification_error(predictions, labels):
    ...
    """
    err = np.where(predictions != labels, 1, 0)

    error = np.sum(err) / len(labels)
    return error
...
def find_strategy_2normal(distribution_A, distribution_B):
    ...
    if p_A < eps:
        ...
        a = (- 1 / (2 * (s_A * s_A))) + (1 / (2 * (s_B * s_B)))
        b = (m_A / (s_A * s_A)) - (m_B / (s_B * s_B))
        c = - ((m_A * m_A) / (2 * s_A * s_A)) + ((m_B * m_B) / (2 * s_B * s_B)) + np.log((s_B * p_A) / (s_A * p_B))
        ...
        if a == 0:
            ...
            D = b * b - 4 * a *c
            if D > 0:
                first = (- b + np.sqrt(D)) / (2 * a)
                second = (- b - np.sqrt(D)) / (2 * a)
                if first < second:
                    q['t1'] = first
                    q['t2'] = second
                else:
...
def bayes_risk_2normal(distribution_A, distribution_B, q):
    ...
    """
    mean_A, sigma_A, prior_A = distribution_A['Mean'], distribution_A['Sigma'], distribution_A['Prior']
    mean_B, sigma_B, prior_B = distribution_B['Mean'], distribution_B['Sigma'], distribution_B['Prior']
    
    
    # 1. Region (-inf, t1]: decision is q['decision'][0]
    if q['decision'][0] == 0:
        integral_1 = prior_A * norm.cdf(q['t1'], loc=mean_A, scale=sigma_A)
    else:
        integral_1 = prior_B * norm.cdf(q['t1'], loc=mean_B, scale=sigma_B)
    
    # 2. Region (t1, t2]: decision is q['decision'][1]
    if q['decision'][1] == 0:
        integral_2 = prior_A * (norm.cdf(q['t2'], loc=mean_A, scale=sigma_A) - norm.cdf(q['t1'], loc=mean_A, scale=sigma_A))
    else:
        integral_2 = prior_B * (norm.cdf(q['t2'], loc=mean_B, scale=sigma_B) - norm.cdf(q['t1'], loc=mean_B, scale=sigma_B))
    
    # 3. Region (t2, inf): decision is q['decision'][2]
    if q['decision'][2] == 0:
        integral_3 = prior_A * (1 - norm.cdf(q['t2'], loc=mean_A, scale=sigma_A))
    else:
        integral_3 = prior_B * (1 - norm.cdf(q['t2'], loc=mean_B, scale=sigma_B))
...
def classify_2normal(measurements, q):
    ...
    label = None
    if q['t1'] == -np.inf:
        label = np.full(q['decision'][0], len(measurements))
    elif q['t1'] == q['t2']:
        label = np.where(measurements <= q['t1'], q['decision'][0], q['decision'][2])
    else:
        label = np.where(
            measurements <= q['t1'], 
            q['decision'][0],  
