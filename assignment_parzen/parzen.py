...
def parzen(x, x_trn, h):
    ...
    p = (1 / x_trn.size) * np.sum(norm.pdf(x.reshape(x.size, -1) - x_trn, 0, h), axis=1)
...
def compute_Lh(itrn, itst, x, h):
    ...
    l = len(itst)
    for i in range (l):  
        sum += np.sum(np.log(parzen(x[itst[i]], x[itrn[i]], h)))
...
def classify_bayes_parzen(x_test, xA, xC, pA, pC, h_bestA, h_bestC):
    ...
    p_xa = parzen(x_test, xA, h_bestA)
    p_xc = parzen(x_test, xC, h_bestC)


    labels = np.where(pA * p_xa >= pC * p_xc, 0, 1)
    return labels
